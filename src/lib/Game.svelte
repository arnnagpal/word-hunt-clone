<script lang="ts">
    ...
    async function checkWord() {
        ...
        let word = selected.map((value) => grid[Math.floor(value / columns)][value % columns]).join('');
        ...
        console.log("Checked word " + word + " and got " + data.found);

        if (data.found) {
            selectionStatus = (wordBank.includes(word)) ? WordSelectionState.OldWord : WordSelectionState.NewWord;
        } else {
            selectionStatus = WordSelectionState.NoWord;
        }
    }
...
{:else}
    <div class="flex h-screen touch-none select-none">
        <div class="grid grid-rows-4 grid-cols-4 gap-3 p-4 m-auto bg-[#4c5f49] border-8 border-[#A4E593] rounded-2xl">
            ...
            "
                    >
                        <p class="[text-shadow:_0px_1px_0_rgb(255_255_255_/_100%)] text-5xl mb-1 font-bold {state[r * columns + c] ? textColors[selectionStatus] : 'text-black'}">{letter}</p>
                    </div>
                {/each}
            {/each}
        </div>
